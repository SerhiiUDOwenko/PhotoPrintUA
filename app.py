...
def get_font(size):
    # Для вебсервера надійно використовувати дефолтний шрифт або завантажений .ttf
    try:
        # Якщо закинеш font.ttf поруч із app.py
...
# --- БЛОК НАЛАШТУВАНЬ (Сайдбар або основне вікно) ---
...
if st.button("Згенерувати PDF", type="primary"):
    if not uploaded_files:
        ...
        with st.spinner('Обробка фотографій та генерація сітки...'):
            # Параметри
            ...
            while q_idx < len(full_queue) or (is_fill_mode and not pages):
                ...
                while True:
                    ...
                    # Беремо ім'я файлу без розширення
            ...
            if pages:
                # Зберігаємо PDF у оперативну пам'ять (не на жорсткий диск)
                pdf_buffer = io.BytesIO()
                pages[0].save(pdf_buffer, format="PDF", save_all=True, append_images=pages[1:], resolution=DPI)
                pdf_bytes = pdf_buffer.getvalue()
                
                st.success("✅ Макет успішно згенеровано!")
                
                # Кнопка для завантаження готового PDF
                st.download_button(
                    label="📥 Завантажити PDF для друку",
                    data=pdf_bytes,
                    file_name=f"Print_Result_{datetime.now().strftime('%H%M%S')}.pdf",
